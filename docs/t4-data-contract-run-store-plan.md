...
This phase does not add live LLM calls. It makes the demo evidence inspectable by future Codex sessions through `data/` and `runs/`.
...
- `runs/comparisons/*.json`: structured summaries linked from artifacts.
...
- A future Codex session can inspect `data/` and `runs/` to explain the baseline failure cluster.
